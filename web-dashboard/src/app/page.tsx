...
export default function Home() {
      ...
      try {
        ...
        TASK_CATEGORIES.forEach((folder) => {
          ...
          if (folderIdx !== -1 && childCount > 0) {
            ...
            wbs[folderIdx].a_start = fmt(minFolderDate);
            if (wbs[folderIdx].progress >= 100) {
              wbs[folderIdx].a_finish = fmt(maxFolderDate);
            }
          }
        });
        setWbsData(wbs);
  ...
  const overallProgress = (() => {
    ...
    return headers.length > 0 ? Math.round(headers.reduce((sum, r) => sum + r.progress, 0) / headers.length) : 0;
  })();
  ...
  const handleEditSave = async (updated: Project) => {
    ...
    if (!error) {
      ...
      setProjects(data || []);
      setSelectedProject(data?.find(p => p.id === updated.id) || null);
    } else {
  ...
  return (
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        {isAdmin && (
          <button onClick={() => setShowAddModal(true)} className="add-project-btn" style={{ margin: '0 20px 12px', padding: '6px 12px', background: 'var(--accent)', color: 'white', border: 'none', borderRadius: '4px' }}>+ Tambah Proyek</button>
        )}
        {projects.length === 0 ? (
          ...
          {loading && wbsData.length === 0 ? (
            ...
            <>
              ...
              <div className="photo-panel">
                ...
                {!selectedRow ? (
                  <div className="empty-state">Pilih salah satu baris pekerjaan (designator) di tabel di atas untuk memuat bukti foto di panel ini.</div>
                ) : selectedRow.evidence.length === 0 ? (
                  <div className="empty-state">Pekerjaan <b>{selectedRow.activity}</b> belum memiliki bukti foto ter-upload.</div>
                ) : (
                  <div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '12px' }}>Menampilkan bukti foto untuk pekerjaan: <b>{selectedRow.activity}</b> ({selectedRow.evidence.length} foto)</div>
                    <div className="photo-grid">
                      {selectedRow.evidence.map((photo, index) => (
                        <div key={index} className="photo-card">
                          <img src={`/api/evident-image?file_id=${photo.file_id}`} alt={`Evident photo ${index + 1}`} loading="lazy" />
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {editProject && (
                <EditProjectModal
                  ...
                  project={editProject}
                  onProjectUpdated={handleEditSave}
                />
              )}
            </>
          )}
      ...
      {/* Add Project Modal */}
      {isAdmin && (
        <AddProjectModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} onProjectAdded={() => {
    ...
    <div className="app-container">
          ...
          <ul className="sidebar-project-list">
            {projects.map(proj => (
              <li key={proj.id}
                ...
                <span className="project-mitra">{proj.nama_mitra}</span>
                {isAdmin && (
                  <button onClick={(e) => { e.stopPropagation(); setEditProject(proj); }} className="edit-btn" style={{ marginLeft: '8px', background: 'transparent', border: 'none', color: 'var(--accent)' }}>✏️</button>
                )}
              </li>
            ))}
          </ul>
      ...
      <main className="main-content">
        <header className="top-navbar">
          ...
          <div style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>User ID: {user?.full_name || 'Guest'}</div>
        </header>
        <div className="content-area">
          {loading && wbsData.length === 0 ? (
            <div className="spinner" />
